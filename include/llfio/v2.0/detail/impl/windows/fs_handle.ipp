  ...
  LLFIO_EXCEPTION_TRY
  {
    for(;;)
    {
      ...
      */
      ...
      HANDLE nh = nullptr;
      NTSTATUS ntstat = NtCreateFile(&nh, SYNCHRONIZE, &oa, &isb, &AllocationSize, 0, fileshare,
                                     0x00000001 /*FILE_OPEN*/, 0x20 /*FILE_SYNCHRONOUS_IO_NONALERT*/, nullptr, 0);
      if(STATUS_SUCCESS != ntstat)
      {
        if(static_cast<NTSTATUS>(0xC000000F) /*STATUS_NO_SUCH_FILE*/ == ntstat ||
           static_cast<NTSTATUS>(0xC0000034) /*STATUS_OBJECT_NAME_NOT_FOUND*/ == ntstat)
        {
          continue;
        }
...
result<void> fs_handle::relink(const path_handle &base, path_view_type path, bool atomic_replace, deadline d) noexcept
{
  ...
  if(!base.is_valid() && !path.is_ntpath())
  {
    ...
    return relink(base, path_view_type(NtPath.Buffer, NtPath.Length / sizeof(wchar_t), path_view::zero_terminated),
                  atomic_replace, d);
  }
  ...
  NTSTATUS ntstat =
  NtSetInformationFile(duph, &isb, fni, sizeof(FILE_RENAME_INFORMATION) + fni->FileNameLength, FileRenameInformation);
...
result<void> fs_handle::link(const path_handle &base, path_view_type path, deadline d) noexcept
{
  ...
  memcpy(fni->FileName, _path.Buffer, fni->FileNameLength);
  NTSTATUS ntstat =
  NtSetInformationFile(duph, &isb, fni, sizeof(FILE_LINK_INFORMATION) + fni->FileNameLength, FileLinkInformation);
  if(STATUS_PENDING == ntstat)
  {
    ntstat = ntwait(duph, isb, d);
  }
...
result<void> fs_handle::unlink(deadline d) noexcept
{
  ...
  if(h.is_directory())
  {
    ...
    if(h.is_symlink())
      ntflags |= 0x00200000 /*FILE_OPEN_REPARSE_POINT*/;
    NTSTATUS ntstat =
    NtOpenFile(&duph, SYNCHRONIZE | DELETE, &oa, &isb, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, ntflags);
  ...
  if(failed)
  {
    if((h.is_regular() || h.is_symlink()) && !(h.flags() & flag::win_disable_unlink_emulation))
    {
      ...
      if(!out)
      {
        ...
        {
          LLFIO_LOG_WARN(this, "Failed to rename entry to random name to simulate immediate unlinking due to "
                               "STATUS_SHARING_VIOLATION, skipping");
        }
        else
        {
...
namespace detail
{
  static inline result<handle> create_alternate_stream(const handle &base, path_view path, handle::mode _mode,
                                                       handle::creation _creation) noexcept
  {
    using namespace windows_nt_kernel;
    ...
    [&]() -> path_view::zero_terminated_rendered_path<>
    {
      return visit(path,
                   [&](auto sv) -> path_view::zero_terminated_rendered_path<>
                   {
                     ...
                     return path_view::zero_terminated_rendered_path<>(
                     path_view(buffer, sv.size() + 1, path_view::not_zero_terminated));
                   });
    }());
    UNICODE_STRING _path{};
    _path.Buffer = const_cast<wchar_t *>(zpath.data());
...
result<span<path_view_component>> fs_handle::list_extended_attributes(span<byte> tofill) const noexcept
{
  ...
  IO_STATUS_BLOCK isb = make_iostatus();
  NTSTATUS ntstat =
  NtQueryInformationFile(h.native_handle().h, &isb, tofill.data(), (ULONG) tofill.size(), FileStreamInformation);
  if(STATUS_PENDING == ntstat)
  {
  ...
  }
  {
    ...
    while(!done && p < tofill.data() + isb.Information)
    {
      ...
      if(i->StreamNameLength > 1 && i->StreamName[0] == ':' && i->StreamName[1] != ':')
      {
        // Alternate data streams can themselves have alternate data streams, so filter out any starting with '$' as
        // those are internal
...
result<span<byte>> fs_handle::get_extended_attribute(span<byte> tofill, path_view_component name) const noexcept
{
  ...
  auto &h = _get_handle();
  LLFIO_LOG_FUNCTION_CALL(&h);
  OUTCOME_TRY(auto &&attribh,
              detail::create_alternate_stream(h, name, handle::mode::read, handle::creation::open_existing));
  IO_STATUS_BLOCK isb = make_iostatus();
  NTSTATUS ntstat = NtReadFile(attribh.native_handle().h, nullptr, nullptr, nullptr, &isb, tofill.data(),
                               (ULONG) tofill.size(), nullptr, nullptr);
  if(STATUS_PENDING == ntstat)
  {
  ...
  */
  ...
  LLFIO_EXCEPTION_TRY
  {
    for(;;)
    {
      ...
      result<handle> ret =
      detail::create_alternate_stream(h, randomname, handle::mode::write, handle::creation::only_if_not_exist);
      if(ret)
      {
  ...
  IO_STATUS_BLOCK isb = make_iostatus();
  NTSTATUS ntstat = NtWriteFile(attribh.native_handle().h, nullptr, nullptr, nullptr, &isb, (PVOID) value.data(),
                                (ULONG) value.size(), nullptr, nullptr);
  if(STATUS_PENDING == ntstat)
  {
    ntstat = ntwait(attribh.native_handle().h, isb, deadline());
  }
  ...
  fni->FileNameLength = _path.Length;
  memcpy(fni->FileName, _path.Buffer, fni->FileNameLength);
  ntstat = NtSetInformationFile(attribh.native_handle().h, &isb, fni,
                                sizeof(FILE_RENAME_INFORMATION) + fni->FileNameLength, FileRenameInformation);
  if(STATUS_PENDING == ntstat)
  {
    ntstat = ntwait(attribh.native_handle().h, isb, deadline());
  }
...
result<void> fs_handle::remove_extended_attribute(path_view_component name) noexcept
{
  ...
  LLFIO_LOG_FUNCTION_CALL(&h);
  OUTCOME_TRY(auto &&attribh,
              detail::create_alternate_stream(h, name, handle::mode::attr_write, handle::creation::open_existing));
  // Mark the item as delete on close
  IO_STATUS_BLOCK isb = make_iostatus();
  FILE_DISPOSITION_INFORMATION fdi{};
  memset(&fdi, 0, sizeof(fdi));
  fdi._DeleteFile = 1u;
  NTSTATUS ntstat =
  NtSetInformationFile(attribh.native_handle().h, &isb, &fdi, sizeof(fdi), FileDispositionInformation);
  if(STATUS_PENDING == ntstat)
  {
    ntstat = ntwait(attribh.native_handle().h, isb, deadline());
  }
...
namespace detail
{
  ...
  static inline result<win_extended_attributes_return_t>
  win_extended_attributes(span<byte> tofill, const handle &h, span<const path_view_component> names) noexcept
  {
    windows_nt_kernel::init();
    using namespace windows_nt_kernel;
    IO_STATUS_BLOCK isb = make_iostatus();
    ...
    NTSTATUS ntstat = NtQueryEaFile(h.native_handle().h, &isb, tofill.data(), (ULONG) tofill.size(), false, ealist,
                                    ealistlen, nullptr, false);
    if(STATUS_PENDING == ntstat)
    {
...
result<span<std::pair<path_view_component, span<byte>>>>
fs_handle::win_list_extended_attributes(span<byte> tofill) noexcept
{
  windows_nt_kernel::init();
  using namespace windows_nt_kernel;
  auto &h = _get_handle();
...
result<span<std::pair<path_view_component, span<byte>>>>
fs_handle::win_get_extended_attributes(span<byte> tofill, span<const path_view_component> names) noexcept
{
  windows_nt_kernel::init();
  using namespace windows_nt_kernel;
  auto &h = _get_handle();
...
result<void>
fs_handle::win_set_extended_attributes(span<std::pair<const path_view_component, span<const byte>>> toset) noexcept
{
  windows_nt_kernel::init();
  using namespace windows_nt_kernel;
  auto &h = _get_handle();
...
LLFIO_HEADERS_ONLY_FUNC_SPEC result<filesystem::path> to_win32_path(const fs_handle &_h,
                                                                    win32_path_namespace mapping) noexcept
{
  windows_nt_kernel::init();
  using namespace windows_nt_kernel;
  ...
  handle checkh(
  native_handle_type(native_handle_type::disposition::file | native_handle_type::disposition::_child_close_executed,
                     CreateFileW(buffer.c_str(), SYNCHRONIZE | FILE_READ_ATTRIBUTES,
